...
from .training_utilities import TrainingMode
import flax
...
def train_step(state, batch, learning_rate_fn,
               cost_func,
               mode=training_utilities.TrainingMode.MULTICLASS,
               mixup_func=None,
    ...
    if isinstance(learning_rate_fn, Callable):
...
def eval_step(state, batch, cost_func, mode=TrainingMode.MULTICLASS):
    ...
    logits, _ = state.apply_fn(
        variables, batch['input'], train=False, mutable=['batch_stats'])
    metrics = training_utilities.compute_metrics(logits, batch['label'], 
                                                 mode=mode, cost_fn=cost_func)
...
def train_and_evaluate(config: ml_collections.ConfigDict,
    ...
    model = training_utilities.create_model(
    ...
    # setup funcs!
    if mode == training_utilities.TrainingMode.MULTICLASS:
    ...
    # TODO: get mixup implementation going
    # mixup_alpha = config.opt.get("mixup_alpha", 0.0)
    # if mixup_alpha != 0.0 and (mode == training_utilities.TrainingMode.MULTILABEL or mode==training_utilities.TrainingMode.MULTICLASS):
    ...
    for step, batch in zip(range(step_offset, num_steps), train_iter):
        ...
        if (step + 1) % steps_per_epoch == 0:
            ...
            if mode == training_utilities.TrainingMode.MULTILABEL:
